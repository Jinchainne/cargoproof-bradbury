...
export default function App() {
  ...
  return <main className="shell">
    ...
    <section className="grid">
      ...
      const escrowBaseUnits = BigInt(Math.round(parseFloat(form.escrow) * 1e18));
      action(e, "create_shipment", [form.shipmentId, form.carrier, form.cargo, form.origin, form.destination, Number(form.window), form.bookingUrl], escrowBaseUnits.toString());
    }}><div className="card-head"><span className="number">01</span><div><h2>Create shipment</h2><p>Buyer action · escrow is sent with this transaction</p></div></div><div className="fields"><label>Shipment ID<input value={form.shipmentId} onChange={(e) => set("shipmentId", e.target.value)} /></label><label>Carrier address<input required placeholder="0x…" value={form.carrier} onChange={(e) => set("carrier", e.target.value)} /></label><label>Cargo description<input value={form.cargo} onChange={(e) => set("cargo", e.target.value)} /></label><label>Delivery window (blocks)<input type="number" min="1" max="100000" value={form.window} onChange={(e) => set("window", e.target.value)} /></label><label>Origin<input value={form.origin} onChange={(e) => set("origin", e.target.value)} /></label><label>Destination<input value={form.destination} onChange={(e) => set("destination", e.target.value)} /></label><label className="wide">Booking / policy evidence URL<input type="url" required value={form.bookingUrl} onChange={(e) => set("bookingUrl", e.target.value)} /></label><label>Escrow (GEN)<input value={form.escrow} onChange={(e) => set("escrow", e.target.value)} /><small>Decimal GEN amount — automatically converted to base units (×10¹⁸) before submission.</small></label></div><button disabled={isBusy} className="primary">Create and lock escrow</button></form>
